...
def canonicalize(candidate: dict[str, Any], source_commit: str, version: int = 1) -> dict[str, Any]:
    """Create a read-only canonical candidate view from a legacy candidate object."""
    ...
    raw_gates = src.get("gates") if isinstance(src.get("gates"), dict) else {}
    ...
    economic = ECONOMIC_MAP.get(decision, "NO_PROVEN_EDGE")
    ...
    return {
        ...
        "supporting_evidence": list(src.get("evidence") or src.get("supporting_evidence") or []),
        "contradictory_evidence": list(src.get("negative_evidence") or src.get("contradictory_evidence") or []),
        "known_failure_patterns": list(src.get("known_failure_patterns") or []),
        "kill_conditions": list(src.get("kill_conditions") or ([src["stop_condition"]] if src.get("stop_condition") else [])),
        "resurrection_conditions": list(src.get("resurrection_conditions") or ([src["resume_condition"]] if src.get("resume_condition") else [])),
        "required_gates": gates,
        "search_family": deepcopy(search),
        "next_decisive_question": src.get("next_decisive_test") or src.get("open_question"),
        "dependencies": list(src.get("dependencies") or []),
        "blockers": list(src.get("blockers") or []),
        "point_in_time_cutoff": src.get("point_in_time_cutoff") or src.get("prospective_cutoff") or src.get("discovery_cutoff"),
