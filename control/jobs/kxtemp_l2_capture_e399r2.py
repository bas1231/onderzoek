...
def main():
 ...
 finished=now_utc()
 record={'schema':'KXTEMP_L2_SNAPSHOT_V1','retrieved_started_at':started.isoformat(),'retrieved_finished_at':finished.isoformat(),'source':'Kalshi public Trade API v2','authenticated':False,'live_trading':False,'discovery_pages':pages,'open_market_count':len(all_markets),'selected_market_count':len(selected),'selected_markets':selected,'books':books}
 ...
 target.write_text(json.dumps(envelope,sort_keys=True,indent=2)+chr(10),encoding='utf-8')
 events=set()
 series=set()
 for market in selected:
  ...
  if ser:
   series.add(ser)
 print('KXTEMP_L2_CAPTURE_E399R2_PASS')
