...
if downloads.is_dir():
    for entry in os.scandir(downloads):
        if entry.is_file() and 'kalshi' in entry.name.lower():
            p = Path(entry.path)
            st = p.stat()
            kalshi_files.append({'path': str(p), 'size': st.st_size, 'mode': oct(st.st_mode & 511)})
...
for venv in venvs:
    ...
    for entry in os.scandir(lib):
        ...
        if sp.is_dir():
            envs.append({'venv': str(venv), 'websockets_present': (sp / 'websockets').exists(), 'cryptography_present': (sp / 'cryptography').exists()})
