...
out=outdir/(stamp+'79905.json')
payload=dict(retrieved_at=datetime.now(timezone.utc).isoformat(),event_id='79905',title=str(group.get('title') or ''),neg_risk_market_id=str(group.get('neg_risk_market_id') or ''),market_count=n,http_status=status,live_trading=False,paid_actions=False,wallet_actions=False,priced=priced,yes_ask_sum=yes_ask,yes_bid_sum=yes_bid,no_ask_sum=no_ask,no_bid_sum=no_bid)
out.write_text(json.dumps(payload,indent=2,sort_keys=True)+chr(10),encoding='utf-8')

print('HOTTEST_NEG_RISK_CAPTURE_PASS')
