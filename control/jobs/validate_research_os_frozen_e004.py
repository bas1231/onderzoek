...
WORKTREE = ROOT / "experiments/bridge/worktrees/research_os_v1_frozen_e004"
PYTHON = ROOT / ".venv/bin/python"
if not PYTHON.exists():
    PYTHON = Path(sys.executable)

EXPECTED_TREES = {
    "control/research_os_v1": "78df2d95d8ee87de42bc06803bd4dc2b776561b6",
    "tests/research_os_v1": "5ea541ac5beb3d409e716d1ee3707fe26b455be6",
...
def find_packet_dir(root: Path) -> Path | None:
    ...
    for path in root.rglob("*.json"):
        try:
            ...
            continue
        # IMPORTANT: filter only the path *inside* the detached checkout. The
        # checkout itself lives under experiments/bridge/worktrees, so checking
        # absolute path.parts would exclude every repository file.
...
try:
    ...
    for path, expected in EXPECTED_TREES.items():
        ...
        ok = actual == expected
        records.append({"label": "frozen_tree_hash", "path": path, "expected": expected, "actual": actual, "required": True, "returncode": 0 if ok else 1})
        if not ok:
            failed = True
    ...
    run("research_os_tests", pytest_base + ["tests/research_os_v1"], cwd=WORKTREE, timeout=600, env=env)
    ...
    run("full_regression_tests", pytest_base + ["tests"], cwd=WORKTREE, timeout=1200, env=env)
...
summary = {
    "validator": "RESEARCH_OS_V1_FROZEN_LOCAL_E004",
...
print("=== RESEARCH_OS_V1_VALIDATION_SUMMARY ===")
