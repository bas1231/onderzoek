...
def atomic_json(path: Path, payload: dict) -> None:
 ...
 tmp.write_text(json.dumps(payload, indent=2, sort_keys=True) + chr(10), encoding='utf-8')
 tmp.replace(path)
...
def run(args: list[str], timeout: int = 20):
 return subprocess.run(args, cwd=ROOT, capture_output=True, text=True, check=False, timeout=timeout)
...
def repair_allowed(key: str, repairs: dict, ts: float) -> bool:
 ...
 return count < MAX_REPAIRS_PER_KEY and ts - last >= REPAIR_COOLDOWN_SECONDS
...
def browser_heartbeat_recent() -> bool:
 proc = run([
 'journalctl', '--user',
 '-u', 'prediction-research-browser-bridge.service',
 '--since', f'{HEARTBEAT_STALE_SECONDS} seconds ago',
 '--no-pager',
 ])
...
def inspect(allow_repairs: bool = True) -> dict:
 ...
 issue = {
 'code': 'PENDING_STALE' if is_committed else 'PENDING_NOT_COMMITTED',
 'task_id': task_id,
 ...
 key = 'pending:' + task_id
 if allow_repairs and repair_allowed(key, repairs, ts):
 ok = restart_service('prediction-research-executor.service')
 note_repair(key, repairs, ts)
 issue['auto_repair_attempted'] = True
 issue['auto_repair_success'] = ok
 emit('AUTO_REPAIR', {'key': key, 'success': ok})
 ...
 bridge_state = load_json(BRIDGE_STATE, {'bridge_tasks': [], 'acked': []})
 acked = set(bridge_state.get('acked', []))

 for task_id in bridge_state.get('bridge_tasks', []):
 if task_id in acked:
 continue
 result_file = ROOT / 'control/results' / task_id / 'RESULT.json'
 if result_file.exists() and age(result_file, ts) > RESULT_STALE_SECONDS:
 issues.append({
 ...
 'auto_repair_attempted': False,
 'note': 'WSL cannot force Chrome content-script reinjection; preserve state and wait for browser reconnect',
 })
...
def loop() -> None:
 ...
 fingerprint = json.dumps(snapshot.get('issues', []), sort_keys=True)
...
if name == 'main':
 ...
 print(json.dumps(inspect(allow_repairs=not args.once), indent=2, sort_keys=True))
