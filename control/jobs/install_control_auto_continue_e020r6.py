...
original_test = TEST.read_text(encoding="utf-8") if TEST.exists() else None
text = original_bridge

if "def _control_continue_result(" not in text:
    anchor = "\ndef next_ai_outbox_item() -> dict | None:\n"
    if anchor not in text:
        raise SystemExit("AUTO_CONTINUE_ANCHOR_MISSING")
    text = text.replace(anchor, "\n" + HELPERS.strip() + "\n\n" + anchor.lstrip("\n"), 1)

next_start = text.index("def next_ai_outbox_item() -> dict | None:")
next_end = text.index("\ndef acknowledge_ai(", next_start)
next_block = text[next_start:next_end]
if "control_item = _next_control_continue_item(" not in next_block:
    needle = (
        "def next_ai_outbox_item() -> dict | None:\n"
        "    state = load_state()\n"
    ...
    if needle not in text:
        raise SystemExit("AUTO_CONTINUE_AI_OUTBOX_ANCHOR_MISSING")
...
if "_queue_control_continue(" not in ack_block:
    ...
    if save_anchor not in ack_block:
        raise SystemExit("AUTO_CONTINUE_ACK_ANCHOR_MISSING")
    ack_block = ack_block.replace(save_anchor, replacement, 1)
...
compile_result = run(sys.executable, "-m", "py_compile", "control/browser_bridge.py")
pytest_result = None
if compile_result.returncode == 0:
    pytest_result = run(sys.executable, "-m", "pytest", "-q", "tests/bridge/test_control_auto_continue.py")

ok = compile_result.returncode == 0 and pytest_result is not None and pytest_result.returncode == 0
...
if LEDGER.exists():
    ...
    marker = "## 2026-09-21 — E020R6 CONTROL_AUTO_CONTINUE"
    if marker not in ledger:
        ledger = ledger.rstrip() + "\n\n" + marker + "\n\n" + (
            "- Server-side durable control continuation is installed behind /ai-outbox.\n"
            "- Normal result ACK queues one continuation item per source task.\n"
            "- Latest CHAT_ACK_STALL can queue the same continuation as fallback.\n"
            "- Malformed queue entries are skipped instead of crashing delivery.\n"
            "- Guardrails keep live trading, paid actions, wallet actions and OpenAI API disabled.\n"
            "- Dedicated regression tests plus py_compile must pass before activation.\n"
            "- Prospective proof still requires this task result to trigger a new AI turn without manual user input.\n"
        )
...
if staged.returncode != 0:
    subprocess.run(["git", "commit", "-m", "fix: install durable control auto continuation"], cwd=ROOT, check=True)
...
print("AUTO_CONTINUE_INSTALL=PASS")
