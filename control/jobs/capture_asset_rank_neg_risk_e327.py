...
for row in manifest.get('groups') or tuple():
    if str(row.get('event_id'))=='106981':
        group=row
        break
if group is None:
...
def top(token):
    ...
    if bids:
        ...
        out.update(bid=float(bid.get('price')),bid_size=float(bid.get('size',0)))
    if asks:
        ask=min(asks,key=lambda x:float(x.get('price',9)))
        out.update(ask=float(ask.get('price')),ask_size=float(ask.get('size',0)))
    return out
...
for market in markets:
    ...
    if not yes or not no or yes.get('bid') is None or yes.get('ask') is None or no.get('bid') is None or no.get('ask') is None:
        complete=False
...
payload=dict(retrieved_at=datetime.now(timezone.utc).isoformat(),event_id='106981',title=str(group.get('title') or ''),market_count=len(markets),http_status=status,live_trading=False,paid_actions=False,wallet_actions=False,complete=complete,priced=priced,books=books,markets=markets)
out.write_text(json.dumps(payload,indent=2,sort_keys=True)+chr(10),encoding='utf-8')

print('ASSET_NEG_RISK_CAPTURE_PASS')
