...
def _parse_ts_ms(value: Any) -> int:
    ...
    if isinstance(value, (int, float)):
        # Treat values above ~2001 in milliseconds as ms, otherwise seconds.
...
def parse_ts_ms(value: Any) -> int:
    """Public timestamp parser used by offline capture loaders."""
...
def _best(levels: Any) -> tuple[Optional[Decimal], Optional[Decimal]]:
    """Return highest bid price and its aggregate quantity."""
    if not levels:
        return None, None
    ...
    for level in levels:
        ...
        if price is None or qty is None:
            raise ValueError("null orderbook level")
        if qty < 0:
            raise ValueError("negative orderbook quantity")
        if qty > 0:
...
class MarketState:
    ...
    def fingerprint(self) -> tuple[Any, ...]:
        # Include top-depth because executable liquidity changes are reactions too.
        return (
    ...
    def as_json(self) -> dict[str, Any]:
        ...
        return {
            "ticker": self.ticker,
            "ts_ms": self.ts_ms,
            "yes_bid": s(self.yes_bid),
            "yes_ask": s(self.yes_ask),
            "no_bid": s(self.no_bid),
            "no_ask": s(self.no_ask),
            "yes_bid_qty": s(self.yes_bid_qty),
            "no_bid_qty": s(self.no_bid_qty),
...
def market_state_from_rest(ticker: str, ts: Any, payload: dict[str, Any]) -> MarketState:
    ...
    # Legacy fallback if a captured fixture uses integer cents.
    ...
    one = Decimal("1")
    yes_ask = None if no_bid is None else one - no_bid
...
class OrderBook:
    """Minimal deterministic Kalshi yes/no bid book reconstruction."""
    ...
    def delta(self, envelope: dict[str, Any], recv_ts: Any) -> MarketState:
        ...
        if envelope.get("type") != "orderbook_delta":
            raise ValueError("not orderbook_delta")
        msg = envelope.get("msg") or {}
        if msg.get("market_ticker") != self.ticker:
            raise ValueError("ticker mismatch")
        seq = envelope.get("seq")
        ...
        price = _dec(msg.get("price_dollars"))
...
def extract_kwi_events(manifests: Iterable[dict[str, Any]], city: str) -> list[dict[str, Any]]:
    """Extract point-in-time E390 first-decision-eligible incomplete signals.

    Eligibility is intentionally copied from the preregistered E390 mechanism:
    the incomplete point must have a numeric target timestamp, all numeric
    station temperatures must equal the previous complete point's contributor
    count, and that previous complete point must precede the target timestamp.
    Availability is the local manifest retrieval time, never the weather
    observation timestamp. Later same-target changes are emitted as revisions.
    """
    ...
    for manifest in manifests:
        ...
        for row in manifest.get("cities") or []:
            ...
            try:
                ...
                previous_v = _dec(previous.get("v"))
                expected = int(previous.get("contributors"))
            ...
            rows.append((retrieved_ms, {
                ...
                "kwi_t": target_t,
                "signal_v": format(mean, "f"),
                "previous_t": previous_t,
                "previous_v": format(previous_v, "f"),
    ...
    for retrieved_ms, event in rows:
        target_t = int(event["kwi_t"])
        fp = tuple(event["station_values"])
        if target_t not in last_fingerprint:
            ...
            kind = "revision"
        prior_fp = last_fingerprint.get(target_t)
        last_fingerprint[target_t] = fp
        out.append({
            **event,
            "kind": kind,
            "available_at_ms": retrieved_ms,
...
def _gap_straddles(gap: dict[str, Any], start_ms: int, end_ms: int) -> bool:
    try:
        gs = _parse_ts_ms(gap["start"])
        ge = _parse_ts_ms(gap["end"])
    except (KeyError, ValueError):
        return True  # malformed gap evidence fails closed
    return not (ge < start_ms or gs > end_ms)


def analyze_reaction(
    kwi_event: dict[str, Any],
    states: Iterable[MarketState],
    trades: Iterable[dict[str, Any]] = (),
    gaps: Iterable[dict[str, Any]] = (),
    coverage_ms: Iterable[int] = (),
    *,
    window_ms: int = 30_000,
    max_pre_age_ms: int = 5_000,
    max_state_gap_ms: int = 5_000,
) -> dict[str, Any]:
    """Classify observed reaction without inferring profitability."""
    t0 = int(kwi_event["available_at_ms"])
    ordered = list(states)
    coverage_points = [int(x) for x in coverage_ms]
    reasons: list[str] = []
    ...
    for a, b in zip(ordered, ordered[1:]):
        if b.ts_ms < a.ts_ms:
            reasons.append("OUT_OF_ORDER_MARKET_STATE")
            break
    ordered = sorted(ordered, key=lambda x: x.ts_ms)

    pre = [s for s in ordered if s.ts_ms <= t0]
    post = [s for s in ordered if s.ts_ms > t0]
    if not pre:
        ...
        if baseline.transport == "ws":
            ...
            recent_evidence = [baseline.ts_ms] + [x for x in coverage_points if x <= t0]
            if t0 - max(recent_evidence) > max_pre_age_ms:
    ...
    # REST polling needs cadence coverage. A quiet WebSocket book emits no
    # deltas by design, so silence is not itself a capture gap; WS gaps must be
    # explicit recorder gap/error events or missing end coverage.
    ...
    for trade in trades:
        try:
            trade_ticker = trade.get("ticker") or trade.get("market_ticker")
            trade_ms = _parse_ts_ms(trade.get("ts_ms", trade.get("created_time", trade.get("ts"))))
        except (AttributeError, ValueError):
            continue
        if trade_ticker == baseline.ticker and t0 < trade_ms <= end:
    ...
    if candidates:
        ...
        return {
            ...
            "economic_conclusion": ECONOMIC_CONCLUSION,
            "reaction_kind": kind,
            "latency_ms": reaction_ms - t0,
            "kwi_event": kwi_event,
            "baseline": baseline.as_json(),
            "reaction_state": obj.as_json() if kind == "book" else obj,
        }

    # To prove no reaction inside a window, capture must cover the full window.
    # WS heartbeat/coverage markers count as transport coverage even when the
    # book is quiet and therefore emits no delta.
    coverage_end = max(
        [s.ts_ms for s in ordered] + coverage_points,
        default=-1,
    )
    if coverage_end < end:
        return {
            "status": UNPROVEN_REACTION,
            "economic_conclusion": ECONOMIC_CONCLUSION,
            "reasons": ["CAPTURE_ENDS_BEFORE_WINDOW"],
            "kwi_event": kwi_event,
            "baseline": baseline.as_json(),
        }
