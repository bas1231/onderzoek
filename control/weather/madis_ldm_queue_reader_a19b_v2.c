/*
 * A19B-v2: read-only LDM product-queue reader.
 *
 * Evidence source:
 *   pq_next() -> queue_par_t.inserted
 *
 * The reader never writes to the LDM product queue.  It emits a compact binary
 * frame on stdout for matching product identifiers; diagnostics go to stderr.
 * Intended production use is as a child of a process started by ldmd EXEC so
 * pq_suspend() can wake immediately on the LDM process-group SIGCONT.  Outside
 * that process group pq_suspend(1) provides a one-second fallback wakeup.
 */
        ...
        reader_state *state)
{
    ...
    if (queue_par->is_locked) {
        flags |= FLAG_PRODUCT_LOCKED;
    }
        ...
        void *restrict app_par)
{
    ...
    /* The cursor starts one second before start_at to avoid a boundary race;
     * products actually inserted before start_at are deliberately ignored. */
