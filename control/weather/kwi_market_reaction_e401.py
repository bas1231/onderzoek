...
class MarketState:
    ...
    def __post_init__(self) -> None:
        for name in ("yes_bid", "yes_ask", "no_bid", "no_ask", "yes_bid_qty", "no_bid_qty"):
...
def analyze_reaction(kwi_event: dict[str, Any], states: Iterable[MarketState],
    ...
    for st in post:
        if st.ts_ms > end: break
        if st.fingerprint != baseline.fingerprint:
            candidates.append((st.ts_ms, "book", st))
    for trade in trades:
        try:
    ...
    coverage_end = max([st.ts_ms for st in ordered] + coverage_points, default=-1)
    if coverage_end < end:
        return {"status": UNPROVEN_REACTION, "economic_conclusion": ECONOMIC_CONCLUSION,
