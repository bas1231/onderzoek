...
src, scanner_count = scanner_pattern.subn(lambda _m: scanner_replacement, src, count=1)
if scanner_count != 1:
    shutil.copy2(BACKUP, PATH)
    die(f"scannerblok niet eenduidig gevonden (matches={scanner_count}); backup hersteld")

# Keep the existing sendCommand HTTP-409 behavior intact.
# Known 0.3.x variants return either false or 'route_conflict' for a 409.
# scanCommands below handles both safely, so no exact-text patch is needed here.
...
scan_replacement = r'''  async function scanCommands() {
    ...
    try {
      ...
      } else if (result === 'route_conflict' || result === false) {
        remember(KEY_COMMAND_IDS, dedupeKey);
      }
    } catch (_) {
...
src, scan_count = scan_pattern.subn(lambda _m: scan_replacement, src, count=1)
if scan_count != 1:
    shutil.copy2(BACKUP, PATH)
