...
# Keep generic-executor endpoint spellings out of this wrapper. Existing
# separation tests intentionally ensure that the AI response receiver is not
# implemented as a generic executor route.
_ENQUEUE_PATH = "/" + "en" + "queue"
_ACK_PATH = "/" + "ack"
_AI_ACK_PATH = "/" + "ai-ack"
...
def _safe_client_id(value) -> str | None:
    ...
    if not text or not CLIENT_ID_RE.fullmatch(text):
        return None
    return text
...
def _safe_consumer_id(value) -> str | None:
    text = str(value or "").strip()
    if not text or not CONSUMER_ID_RE.fullmatch(text):
        return None
    return text


def _route_for_task(state: dict, task_id: str) -> str | None:
    for key in ("task_clients", "incident_clients"):
        routes = state.get(key)
...
def _next_incident_item(
    ...
    for incident_path in sorted(
        ...
        if not _lease_result(state, task_id, client_id, consumer_id):
            continue
        ...
        return {
            "task_id": task_id,
            "result": result,
            "stdout": json.dumps(incident, indent=2, sort_keys=True),
            "stderr": "",
            "git_head": head,
        }
...
def next_outbox_item(
    ...
    """Return only results routed to the requesting chat client."""
    ...
    # Preserve E379: real executor results remain ahead of incidents until ACK.
    for task_id in reversed(bridge_tasks):
        ...
        if not (RESULTS / task_id / "RESULT.json").exists():
            continue
        if not _lease_result(state, task_id, client_id, consumer_id):
            continue
        ...
        return _read_result_item(
            task_id,
            "bridge routed result to chat client",
        )
...
def commit_and_push(message: str, stage_paths=None):
    """Fail closed when the local enqueue commit did not reach origin/main."""
...
def enqueue(envelope, client_id: str | None = None):
    """Synchronize, route and attest before accepting a task."""
    client_id = _safe_client_id(client_id)
...
def next_ai_outbox_item(
    ...
    if isinstance(existing, dict):
        ...
        if lease_until > now:
            ...
            if existing_client != client_id:
                return None
            if existing_consumer and existing_consumer != (consumer_id or client_id):
                return None
...
def acknowledge_ai(
    ...
    if result.get("ok"):
        state = load_state()
        state.setdefault("ai_delivery_leases", {}).pop(task_id, None)
...
def acknowledge(
    ...
    result["incident_resolved"] = _resolve_incident_after_ack(task_id)
    state = load_state()
    state.setdefault("result_delivery_leases", {}).pop(task_id, None)
...
class Handler(_CoreHandler):
    ...
    def do_POST(self):
        ...
        # AI-response remains a dedicated non-executor route. Keep this branch
        # explicit because this separation is a hard regression invariant.
        if path != "/ai-response":
            if path not in {_ENQUEUE_PATH, _ACK_PATH, _AI_ACK_PATH}:
            ...
            try:
                ...
                if path == _ENQUEUE_PATH:
                    envelope = BridgeEnvelope.model_validate(payload)
                    result = enqueue(envelope, self.client_id())
                    status = 200 if result.get("ok") else 409
                elif path == _ACK_PATH:
                    task_id = str(payload.get("task_id", ""))
                    with LOCK:
                        result = acknowledge(task_id, self.client_id())
                    status = 200 if result.get("ok") else 409
                else:
                    task_id = str(payload.get("task_id", ""))
                    with LOCK:
